import { api } from './api';
import type { Prenotazione, StoricoStato, DatiCarico } from '@planner-molino/shared';
...
export const prenotazioniService = {
  list: (params?: Record<string, string>) => {
    ...
    return api.get<PaginatedResponse>(`/prenotazioni${query}`);
  },
  ...
  getById: (id: number) => api.get<PrenotazioneDetail>(`/prenotazioni/${id}`),
  ...
  getDatiCarico: (id: number) => api.get<DatiCarico>(`/prenotazioni/${id}/dati-carico`),
